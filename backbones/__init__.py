...
def get_model(name, **kwargs):
    ...
    if name == "r18":
        ...
        return SwinTransformer(img_size=112, patch_size=2, num_classes=num_features, embed_dim=48)
    elif name == "swin_s":
        # this is a feature
        num_features = kwargs.get("num_features", 512)
        from .swin_transformer import SwinTransformer
        return SwinTransformer(img_size=112, patch_size=2, num_classes=num_features, embed_dim=48, depths=[2, 2, 18, 2])
    elif name == "swin_b":
        # this is a feature
        num_features = kwargs.get("num_features", 512)
        from .swin_transformer import SwinTransformer
        return SwinTransformer(img_size=112, patch_size=2, num_classes=num_features, embed_dim=64,
                               depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32])
    elif name == "swin_l":
        # this is a feature
        num_features = kwargs.get("num_features", 512)
        from .swin_transformer import SwinTransformer
        return SwinTransformer(img_size=112, patch_size=2, num_classes=num_features, embed_dim=96,
                               depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48])
    elif name == "swin_mgface_only_unet":
        num_features = kwargs.get("num_features", 512)
        from .mgface_only_unet import STMGFACE
        return STMGFACE(img_size=112, patch_size=2, num_classes=num_features, embed_dim=64, depths=[2, 2, 18, 2],
                       num_heads=[4, 8, 16, 32])
    elif name == "swin_mgface_only_cls":
        num_features = kwargs.get("num_features", 512)
        from .mgface import STMGFACE
        return STMGFACE(img_size=112, patch_size=2, num_classes=num_features, embed_dim=64, depths=[2, 2, 18, 2],
                       num_heads=[4, 8, 16, 32], mask_method=None)
    elif name == "swin_mgface":
        num_features = kwargs.get("num_features", 512)
        from .mgface import STMGFAE
        return STMGFACE(img_size=112, patch_size=2, num_classes=num_features, embed_dim=64, depths=[2, 2, 18, 2],
                       num_heads=[4, 8, 16, 32], mask_method="all")
    else:
        raise ValueError()
